...
NOTE: Here, $q(x_t|x_0) = \mathcal(N)(x_t; \sqrt(\bar{\alpha}_t)x_0, (1- \sqrt(\bar{\alpha}_t)I))$ is called Denoising Score Matching (DSM)

NOTE: Here, $p(x)$ is not easily accessible for our data, so we can obtain it via flow matching techniques. (idk if this is relevant here?)
