...
- Describing complex underlying distrubution effectively with limited sampling
    ...
    - For a given z, find the P(x|z).
    - Joint probability is: $P(x, z) = P(z) * P(x|z)$
- How to represent distribution?
    ...
    - NN is outputting the parameters of the prob.distribution. Hence, for a sample z, we out a probability P(x|z)
- Objective is to minimize the error (some distance) between distribution  $P_data or P(x)$ and $P_model or P_\theta (x)$
    ...
    - Now, estimate $P_\theta (x)$ -> we can get $P_\theta (x|z)$ using latent to outputting distribution parameters.
    - Many different z values could have generated a specific x value, so need to consider all possible latent values: $P_\theta (x) = \sum_z P_\theta (x|z)P_\theta(z)$ (called likelihood)
...
3. Map each $z_k$ through the NN and get parameters for the $P_\theta(x|z_k)$ for each $z_k$.
4. For each $z_k$ compute the $P_\theta(x|z_k)$
5. Compute average log-likelihood $log[\sum_z^{z_k} P_\theta (x|z)P_\theta(z)]$
...
- Better to oversample rare and important cases, and mathamatically correct for the bias, rather then risk missing them entirely!
    ...
    - This network learns to predict which region in the latent space have likely generated each data point.
        - NN leans a variational distribution: $Q_theta (z|x)$
    - Can try optimize the parameters of $Q_\theta$ and $P_\theta$ directly by maximizing: $log P_theta (x) = log E_{z~Q_\theta (z|x)} [P_\theta (x|z) \frac{P_\theta(x|z)}{Q_\theta(z|x)}]$ but is probelematic.
...
- Jensen's Inequality:
    ...
    - Has intuitive meaning for the terms:
        -Accuracy term : $E[logP_\theta(x|z)]$
        -Complexity term: $D_{KL}[Q_\theta(z|x)||P_\theta(z)]$
...
- Only learn the weight of the recognition model $Q_\theta(z|x)$ and the generative model $P_\theta (x|z)$

## Updated Training loop
0. Take a data point (x) from training dataset
1. Pass through NN of recognition model to get the parameters of the importance sampling model $Q_\theta(z|x)$.
...
5. Compute the ELBO objective (which is to be maximized):
    $$E[logP_\theta(x|z)] - D_{KL}[Q_\theta(z|x)||P_\theta(z)]$$
...
The accuracy formula in ELBO is proportiaonal  to the following: $log P_\theta (x|z) ~ - ||x-\mu||^2$ with the isotropic gaussian assumption!
