...
To collect demonstration trajectories, a simple keyboard teleop interface can be created in ROS2 (or someother realtime framework) where the observations from the camera and the commands sent to the robot arm (joint angle velocity) is recorded in a ROS Bag (or a csv file) which can be then used to train the network. 
