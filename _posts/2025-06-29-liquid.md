...
- Encoder (E): Learns to represent the latent spatial component, typically via a variational encoder. It is typically written as (where $z_t$ is the latent state, and $o_t$ is the observation at time t):
$$ z_t ~ Q_\psi (z_t| \hat{z_t}, o_t)$$

It is analagous to a Kalman Filter.

- Transition Model (TM): Learns to predict the temporal evolution of the latent states (typically via a RNN). It is represented as follows:
$$\hat{z_{t+1}} ~ p_\theta(\hat{z_{t+1}} | z_t, a_t)$$
...
$$\mathcal{L}(\mu) = 
\frac{1}{N_{\text{safe}}} \sum_{o^+ \in \mathcal{D}_{\text{safe}}} \mathrm{ReLU}\left(\delta - \ell_{\mu}(\mathcal{E}_{\psi}(o^+))\right)
+ \frac{1}{N_{\text{fail}}} \sum_{o^- \in \mathcal{D}_{\text{fail}}} \mathrm{ReLU}\left(\delta + \ell_{\mu}(\mathcal{E}_{\psi}(o^-))\right)$$
...
- DDPG is used for approximating the HJ-Value Function

(Refer to * for more information about an overview of HJ-Reachability.)

#### Data Collection

For world model training:
- 1000 offline trajectory
- 750 from sampling random action from a Gaussian Distribution
- 125 trajectories which lead to spill (label the spill states)
- 125 trajectories which do not spill
