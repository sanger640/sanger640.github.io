...
Usually for this approach, the perception and planning module are combined to create something called a visuomotor policy. A visuomotor policy takes in visual observations (RGB frames) and directly outputs coordinates in end-effector space or joint angles; these values are then passed to the low-level controllers present on the robot arm. A policy can also be made to directly output motor torque values, but generally this is a rare practice due to safety concerns. 
...
To collect demonstration trajectories, a simple keyboard teleop interface can be created in ROS2 where the observations from the camera and the commands sent to the robot arm (joint angle) is recorded in a ROS Bag which can be then used to train the network. 
...
- Finetune output layers, with final layer having 7 outputs (joint angles)

#### Diffusion Model

### Implementing safety

- CBF/Reachability Analysis from IL trajectory
- World Models
- Heurestic Safety
