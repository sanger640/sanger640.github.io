...
(**Important note**: This article is primarily based on the paper by <d-cite key="safeFilter"></d-cite>)

### What is safety? How to represent it?
...
## Hamilton-Jacobi Reachability analysis (using Dynammic Programming)

Hamilton-Jacobi (HJ) Reachability Analysis is a framework for identifying when present actions will cause future failtures and for computing best policies that minimizes failures. It assumes access to the state space, bounded non-linear dynamics function, and a compact action/control set. A domain expert is also needed to first specify what safety is in a given context by specifying a failure set on the state space. 

Given this scenario, HJ reachability will compute two things:
1. Safety Monitor (V) which quantifies if the robot is in an unsafe state (doomed to collide)
2. Safety Preserving Policy ($\pi$)




More formally, it allows the computation of a least restrictive or maximal safe set using backward induction principles from dynamic programing (DP). HJ reachability analysis can be though of as a pursuit-evasion game between the controller trying to avoid failure set while an adverserial agent aims to steer the system into the failure set. It is formulated for discrete time as follows:

$$$$

In essence, it is analgous to Value Iteration, where you try to find the value of the states, given the immediate rewards are accessible. Similarly, using DP the above reachability function will converge for each state, given the initial distance between the obstacles and the current state is defined. Look at Figure * for a better illustration.

In practice, results in control chatter and erratic motion near safe set boundary due to constant switching between the fall-back policy and the task policy. Furthermore, due to DP requiring discretization this method is quite resourse intensive and unrealastic for systems with more than six continuous state variables. 
