...
For our example of lateral path tracking, the states are the lateral offset from the path, the slip angle (angle between the tires heading and the direction vehicle is moving), and the yaw rate. The model which describes the evolution of these parameters is called the single track, linear dynamic bicycle model (as seen in Figure 3) <d-cite key="Schildbach2020"></d-cite>.
